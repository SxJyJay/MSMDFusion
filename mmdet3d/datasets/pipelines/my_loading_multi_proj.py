...
class GlobalRotTransFilterForeground2D(object):
    ...
    def _synchronize_pcd_DA(self, input_dict):
        ...
        fg_real_pixels = fg_info['fg_real_pixels']
...
class ImgScaleCropFlipForeground2D(object):
    ...
    def __call__(self, input_dict):
        ...
        real_pxl = input_dict['foreground2D_info']['fg_real_pixels']
        cam_num = len(pxl)
        for i in range(cam_num):
            pxl_per_cam = pxl[i]
            real_pxl_per_cam = real_pxl[i]
            pxl_per_cam[:, :2] = pxl_per_cam[:, :2] * img_scale_factor
            real_pxl_per_cam[:, :2] =  real_pxl_per_cam[:, :2] * img_scale_factor
            pxl_per_cam -= img_crop_offset
            real_pxl_per_cam -= img_crop_offset
            if img_flip:
                orig_h, orig_w = img_shape
                pxl_per_cam[:, 0] = orig_w - 1 - pxl_per_cam[:, 0]
                real_pxl_per_cam[:, 0] = orig_w - 1 - real_pxl_per_cam[:, 0]
            pxl[i] = pxl_per_cam
            real_pxl[i] = real_pxl_per_cam
                
        input_dict['foreground2D_info']['fg_pixels'] = pxl
        input_dict['foreground2D_info']['fg_real_pixels'] = real_pxl

        return input_dict

@PIPELINES.register_module()
class ShuffleForeground2D(object):
    """Shuffle fg_pixel and fg_points to prevent overfitting

        Note: Must guarantee shuffle synchronization between fg_pixel and fg_points
    """
    def __init__(self, **kwargs):
        pass
    ...
    def __call__(self, input_dict):
        ...
        fg_pixels = input_dict['foreground2D_info']['fg_pixels']
        fg_points = input_dict['foreground2D_info']['fg_points']
        cam_num = len(fg_pixels)
        for i in range(cam_num):
            fg_pxl = torch.from_numpy(fg_pixels[i])
            fg_pts = fg_points[i].tensor
            
            assert fg_pxl.shape[0] == fg_pts.shape[0]
            rand_perm_seq = torch.randperm(fg_pts.shape[0])
            fg_pxl = fg_pxl[rand_perm_seq]
            fg_pts = fg_pts[rand_perm_seq]
            
            fg_pixels[i] = fg_pxl.numpy()
            fg_points[i].tensor = fg_pts
