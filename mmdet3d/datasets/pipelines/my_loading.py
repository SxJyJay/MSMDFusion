...
class LoadForeground2D(object):
    ...
    def _organize(self, fg_info):
        ...
        for i in range(cam_num):
            ...
            _, unique_indices = np.unique(fg_pixel_indices, return_index=True, axis=0)
            unique_fg_pixels = fg_pixel_indices[unique_indices]
            unique_fg_points = fg_points_set[unique_indices]
            # point_class = get_points_type('LIDAR')
            # unique_fg_points = point_class(
            #     unique_fg_points, points_dim=unique_fg_points.shape[-1]
            # )
            fg_pixels.append(unique_fg_pixels)
            fg_points.append(unique_fg_points)
    ...
    def __call__(self, results):
        if self.dataset == 'NuScenesDataset':
            ...
            fg_path = os.path.join(*tokens[:-2], "FOREGROUND_MIXED", tokens[-1]+'.pkl.npy')
    ...
    def _merge_sweeps(self, fg_info, sweep_fg_info, sweep):
        ...
        if len(sweep_fg_points) == len(fg_points):
            ...
            for cam_id in range(cam_num):
                ...
                sweep_fg_point = sweep_fg_point @ sweep['sensor2lidar_rotation'].T
                sweep_fg_point += sweep['sensor2lidar_translation']
    ...
    def __call__(self, results):
        if self.dataset == "NuScenesDataset":
            ...
            for idx in choices:
                ...
                tokens = pts_filename.split('/')
                sweep_fg_path = os.path.join(*tokens[:-2], "FOREGROUND_MIXED", tokens[-1]+'.pkl.npy')
