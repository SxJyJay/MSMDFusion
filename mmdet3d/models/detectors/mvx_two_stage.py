...
class MVXTwoStageDetector(Base3DDetector):
    ...
    def __init__(self,
                 ...
                 pts2D_voxel_encoder=None,
                 pts_middle_encoder=None,
                 pseudo_pts_middle_encoder=None,
                 multimodal_middle_encoder=None,
        ...
        self.discard_views = discard_views
        ...
        if pts_voxel_encoder:
            self.pts_voxel_encoder = builder.build_voxel_encoder(
                pts_voxel_encoder)
        if pts2D_voxel_encoder:
            self.pts2D_voxel_encoder = builder.build_voxel_encoder(
                pts2D_voxel_encoder)
        if pts_middle_encoder:
            self.pts_middle_encoder = builder.build_middle_encoder(
                pts_middle_encoder)
        if pseudo_pts_middle_encoder:
            self.pseudo_pts_middle_encoder = builder.build_middle_encoder(
                pseudo_pts_middle_encoder)
        if multimodal_middle_encoder:
            self.multimodal_middle_encoder = builder.build_middle_encoder(
                multimodal_middle_encoder)
        if pts_backbone:
        ...
        ## donnot init weights for CenterPoint Kitti
