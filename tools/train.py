...
def main():
    ...
    if 'freeze_lidar_components' in cfg and cfg['freeze_lidar_components'] is True:
        ...
        ## only freeze pts encoder before bev map
        for name, param in model.named_parameters():
            if 'pts_middle_encoder' in name:
                param.requires_grad = False
            if 'pts_voxel_layer' in name:
                param.requires_grad = False
            if 'pts_voxel_encoder' in name:
                param.requires_grad = False

            if 'MSMD_CenterPoint' in cfg.model.type:
    ...
    if 'freeze_det_head' in cfg and cfg['freeze_det_head'] is True:
        logger.info(f"param need to update:")
        param_grad = []
        param_nograd = []

        for name, param in model.named_parameters():
            if 'pts' in name and 'pts_bbox_head' in name:
                param.requires_grad = False
        ...
        def fix_bn(m):
            if isinstance(m, nn.BatchNorm1d) or isinstance(m, nn.BatchNorm2d):
                m.track_running_stats = False
        if 'TransFusion' in cfg.model.type:
